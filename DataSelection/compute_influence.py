...
def prepare_gradients_train():
    ...
    for i in range(4):
        ...
        for j in range(num):
            gradients = torch.load(os.path.join(gradients_path, f"gradient_{j+1}.pt", map_location="cuda:1"))
...
def prepare_gradients_evaluation():
    ...
    num = len([f for f in os.listdir(gradients_path)])
    for j in range(num):
        gradients_eval.append(os.path.join(gradients_path, f"gradient_{j+1}.pt"))
