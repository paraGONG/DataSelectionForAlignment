...
class PPOTrainer(ABC):
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        self.output_save_path = f"./case_study_rank_{self.strategy.get_rank()}"
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts in self.prompts_dataloader:
                ...
                if steps % update_timesteps == 0:
                    ...
                    eval_mean_reward = self.compute_eval_gradients(args, eval_prompts, steps)
                    # compute influence
    ...
    def compute_eval_gradients(self, args, eval_prompts, steps):
        # make eval experience
        ...
        for prompt in tqdm(eval_prompts):
            ...
            # output = self.tokenizer.batch_decode(experience.sequences, skip_special_tokens=True)
            # save here
            # with open(os.path.join(self.output_save_path, 'output_steps.jsonl'), 'a') as f:
            #     data = {'data': 'evaluation', 'output': output[0], 
            #             'reward': experience.info["reward"].item(), 'advantage': experience.advantages}
            #     json.dump(data, f)
            #     f.write('\n')
    ...
    def compute_influence(self, global_steps):
    ...
    def data_sorting(self, steps):
        items = self.replay_buffer.items
        zipped = list(zip(self.influence_scores, items, self.actor_loss))
        sorted_zipped = sorted(zipped, key=lambda x: x[0], reverse=True)
        sorted_influences, sorted_items, sorted_losses = zip(*sorted_zipped)
        sorted_influences = list(sorted_influences)
        sorted_items = list(sorted_items)
        sorted_losses = list(sorted_losses)

        top_20_data = zip(sorted_influences[:20], sorted_items[:20], sorted_losses[:20])
        bottom_20_data = zip(sorted_influences[-20:], sorted_items[-20:], sorted_losses[-20:])

        # # save here
        # with open(os.path.join(self.output_save_path, f'validation_steps_{steps}.jsonl'), 'a') as f:
        #     data = {'data': 'validation', 'output': output[0], 'loss': loss.item()
        #             'reward': experience.info["reward"].item(), 'return': experience.info["return"].item(), 
        #             'advantage': experience.advantages.mean().item(), 'value': experience.values.mean().item(),
        #             'response_length':experience.info['response_length'].item(), 'total_length':experience.info['total_length'].item()}
