deepspeed --include localhost:3 --master_port 29503 --hostfile=None --module openrlhf.cli.calculate_gradient \
  --pretrain  TinyLlama/TinyLlama-1.1B-Chat-v1.0 \
  --reward_pretrain yifangong/TinyLlama-1.1B-Chat-v1.0-reward-model \
  --save_path ../tinyllamachat_global_step10_gradients_train_part_3 \
  ...
  --init_kl_coef 0.01 \
  --prompt_data yifangong/split_candidate_dataset_part_3 \
  --input_key prompt \
  --input_template "<|user|>\n{}</s>\n<|assistant|>\n" \
  --max_samples 100000 \
  --load_checkpoint \
